...
const Home: React.FC<HomeProps> = ({ 
  user, myDeposits, performCheckIn, onOpenWithdraw, onOpenDeposit, onOpenWheel,
  ...
  return (
    <div className="p-5 space-y-6 animate-in fade-in duration-700 bg-slate-50 min-h-screen">
      {/* Header com ID Único e Toggle de Moeda */}
      <div className="flex items-center justify-between px-2 pt-2">
        <div className="flex flex-col">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Identificador de Ativação</p>
          <div className="flex items-center space-x-2">
            <span className="text-sm font-black text-slate-900 tracking-tighter">ID: {user.referralCode}</span>
            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
          </div>
        </div>
        
        {/* Switch de Moeda Moderno */}
        <button 
      ...
      <div className="relative overflow-hidden bg-slate-900 rounded-[35px] p-7 text-white shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center opacity-50 mb-1">
            <span className="text-[10px] font-black uppercase tracking-[0.2em]">Balanço {currency}</span>
          </div>
          <div className="flex items-baseline space-x-2 mb-8">
            <h2 className="text-4xl font-black tracking-tighter">
              {formatValue(user.balance).replace('R$', '').trim()}
            </h2>
            <span className="text-emerald-400 font-bold text-xs uppercase">{currency.toLowerCase()}</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={onOpenDeposit} className="bg-emerald-500 text-slate-900 font-black py-4 rounded-2xl text-[10px] uppercase active:scale-95 transition-all shadow-lg">Ativar Plano</button>
            <button onClick={onOpenWithdraw} className="bg-white/10 text-white font-black py-4 rounded-2xl text-[10px] uppercase border border-white/10 active:scale-95 transition-all">Sacar Lucros</button>
          </div>
        </div>
      </div>
      ...
      <div 
        ...
        className="bg-white border-2 border-[#25D366]/30 rounded-[35px] p-5 shadow-lg shadow-green-100 flex items-center justify-between group active:scale-[0.98] transition-all cursor-pointer overflow-hidden relative"
      >
        <div className="absolute top-0 right-0 w-24 h-full bg-[#25D366]/5 -skew-x-12 group-hover:bg-[#25D366]/10 transition-all"></div>
        <div className="flex items-center space-x-4 relative z-10">
          <div className="w-12 h-12 bg-[#25D366] text-white rounded-2xl flex items-center justify-center shadow-lg shadow-green-200">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Comunidade Oficial</p>
            <p className="text-sm font-black text-[#25D366] uppercase italic tracking-tighter">ENTRAR NO GRUPO VIP</p>
          </div>
        </div>
        <div className="w-10 h-10 rounded-full bg-[#25D366]/10 flex items-center justify-center text-[#25D366] relative z-10 animate-pulse">
      ...
      <DailyCheckIn user={user} onCheckIn={performCheckIn} currency={currency} />
      ...
      <div className="space-y-3">
        <div className="bg-white border border-slate-100 p-5 rounded-[28px] flex items-center justify-between shadow-sm relative overflow-hidden">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Operação Vigente</p>
