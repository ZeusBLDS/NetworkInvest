...
const NetworkView: React.FC<NetworkProps> = ({ user }) => {
  const referralLink = `https://networkinvest.com/reg?ref=${user.referralCode}`;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase">Seu Link de Indicação</p>
            <p className="text-sm font-semibold text-gray-800 truncate max-w-[200px]">{referralLink}</p>
          </div>
        </div>
        
        <button 
          onClick={copyLink}
          className="w-full bg-emerald-600 text-white font-bold py-3 rounded-2xl flex items-center justify-center space-x-2 active:scale-95 transition-all shadow-md shadow-emerald-50"
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm text-center">
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase">Total Indicados</p>
        </div>
        <div className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm text-center">
          <p className="text-2xl font-black text-emerald-600">0.00</p>
          <p className="text-[10px] font-bold text-gray-400 uppercase">Ganhos de Rede</p>
        </div>
      </div>

      <div className="space-y-3">
        <h3 className="text-sm font-bold text-gray-800 px-1">Ganhos por Nível</h3>
        <div className="space-y-2">
          {stats.map((stat) => (
            <div key={stat.level} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center font-bold text-gray-500 text-xs">
                  {stat.level}º
                </div>
                <div>
              ...
              <div className="text-right">
                <p className="font-bold text-gray-800">{stat.count} <span className="text-[10px] text-gray-400 font-medium">indicados</span></p>
